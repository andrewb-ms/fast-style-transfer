...
@app.route('/tensorflow/monet', methods=['POST'])
def tensorflow_monet():
    if request.method == 'POST':
        # check if the post request has the file part
        if 'file' not in request.files:
            #flash('No file part')
        ...
        if file and allowed_file(file.filename):
            ...
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            os.system('python evaluate.py --checkpoint /msshared/models/fastmodels/multistyle-pastiche-generator-monet.ckpt --in-path /msshared/tensorflow/ --out-path /msshared/tfoutput/')
            return send_file(os.path.join('/msshared/tfoutput/', filename), mimetype='image/' + os.path.splitext(filename)[1][1:])
